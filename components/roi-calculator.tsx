...
export function ROICalculator() {
  ...
  const results: ResultsType = useMemo(() => {
    ...
    for (let year = 1; year <= inputs.careerLength; year++) {
      ...
      cumulativeEarnings += nominalEarnings
      ...
      const yearlyROI =
        year === 1
          ? 0
          : ((nominalEarnings -
              inputs.startingSalary * Math.pow(1 + growthRate, year - 2)) /
              totalEducationCost) *
            100
      ...
      chartData.push({
        ...
        cumulativeEarnings,
        netValue,
        yearlyEarnings: nominalEarnings,
        yearlyROI,
      })
    ...
    const projectedSalary =
      inputs.startingSalary * Math.pow(1 + growthRate, inputs.careerLength - 1)
        ...
        <CardContent>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={results.costBreakdown as unknown as { name: string; value: number }[]}
                  dataKey="value"
                  nameKey="name"
                  outerRadius={100}
                  label
